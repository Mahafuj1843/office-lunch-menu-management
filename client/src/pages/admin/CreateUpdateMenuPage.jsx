...
const CreateUpdateMenuPage = () => {
    ...
    var Menu = useSelector((state) => (state.menu.menuDetails));
    ...
    const handleUpdate = async (e) => {
        ...
        if (IsEmpty(menu.title)) {
            ...
            const result = await updateMenuRequest({ ...menu, extras: extraOptions }, param.id)
                ...
                <form onSubmit={param?.id ? handleUpdate : handleSubmit} className='w-full space-y-2 md:w-[40%] mx-auto py-4'>
                    ...
                    <div className='space-y-1'>
                        <div className='space-y-1 w-full'>
                            ...
                            <input onChange={handleChange} value={menu.title} name="title" type="text" id='title' class="bg-gray-50 border border-gray-300 text-gray-900 text-sm outline-none rounded-lg focus:ring-gray-300 focus:border-gray-300 block w-full px-2 py-1.5" />
                        </div>
                        <div className='space-y-1 w-full'>
                            <label className='text-base font-semibold' htmlFor="desc">Description *</label>
                            <input onChange={handleChange} value={menu.desc} name="desc" type="text" id='desc' class="bg-gray-50 border border-gray-300 text-gray-900 text-sm outline-none rounded-lg focus:ring-gray-300 focus:border-gray-300 block w-full px-2 py-1.5" />
                        </div>
                        <div className='space-y-1 w-full'>
                            <label className='text-base font-semibold' htmlFor="date">Date *</label>
                            <input onChange={handleChange} value={menu.mDate?.split("T")[0]} name="mDate" type="date" id='date' class="bg-gray-50 border border-gray-300 text-gray-900 text-sm outline-none rounded-lg focus:ring-gray-300 focus:border-gray-300 block w-full px-2 py-1.5" />
                        </div>
                        <div className='space-y-1 w-full'>
